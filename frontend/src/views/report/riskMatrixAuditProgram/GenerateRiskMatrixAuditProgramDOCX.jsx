...
export const GenerateRiskMatrixAuditProgramDOCX = async (data) => {
            ...
            new TableCell({
              ...
              children: [
                new Paragraph({
                  ...
                  children: [
                    new TextRun({
                      text: `Matriz de riesgo | ${data.name} - FY ${data.fiscal_year}`,
                      bold: true,
                      size: 28,
                      color: "FFFFFF",
                    }),
  ...
  const rows1 = data.risks.map((r, idx) => {
  ...
  const rows2 = data.risks.map((r, idx) => {
    const zebra = idx % 2 === 1 ? "F2F2F2" : undefined;
    return new TableRow({
      children: [
        dataCell({ text: r.slug, fill: zebra, outerBorder: true }),
  ...
  const footer = [
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: `Total de riesgos: ${data.risks.length}`,
          bold: true,
          size: 22,
        }),
      ],
  ...
  const fileName = `matriz_de_riesgo_${data.fiscal_year}_${safeTitle}.docx`;
