...
import { countryCodes, statusOptions } from "@/utils/HelpersLib";
...
const UserUpdate = () => {
  ...
  const fetchUser = async () => {
    try {
      ...
      const validateRole = (vRole) => {
        if (userData.role.status === false) return null;
        return vRole;
      };
      setValue("first_name", userData.first_name);
      setValue("last_name", userData.last_name);
      setValue("email", userData.email);
      setValue("countryCode", userData.countryCode || "+58");
      setValue("phone", cleanPhone(userData.phone));
      setValue("id_role", validateRole(userData.id_role));
      setValue("status", userData.status);
    } catch (error) {
      setError(error.message || "Error al cargar datos del usuario o roles.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    setSuccess("");
    try {
      const successMessage = await updateUser(id, formData);
  ...
  return (
    ...
    >
      <Paper elevation={10} sx={{ padding: 4, width: 500 }}>
        <Stack spacing={3} alignItems="center">
          ...
          >
            ...
            <ButtonDesign>Actualizar Usuario</ButtonDesign>
          </form>
      ...
      <ConfirmModalDesign
        ...
        title="Confirmación del actualizar el usuario"
        message="¿Estás seguro de que deseas actualizar los datos del usuario?"
