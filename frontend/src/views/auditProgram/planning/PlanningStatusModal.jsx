...
import { updatePlanningStatus } from "@/services/auditProgram/Planning";
import { statusPhasesOptions } from "@/utils/HelpersLib";

export default function PlanningStatusModal({ open, onClose, onStatusSuccess}) {
  const {
    handleSubmit,
    control,
    setValue,
  ...
  const fetchAuditProgram = async () => {
    try {
      ...
      setValue("status", auditProgram.planning_status);
    } catch (error) {
  ...
  const onSubmit = async (formData) => {
    try {
      const message = await updatePlanningStatus(formData, slug);
      ...
      setSnackbarSeverity("error");
      setSnackbarMessage(
        error.message || "Error al actualizar el estatus de planificación."
      );
      setSnackbarOpen(true);
    } finally {
      setLoading(false);
