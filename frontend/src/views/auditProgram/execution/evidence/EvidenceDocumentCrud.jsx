...
import {
  ...
  getEvidenceDownloadDocuments,
  deleteEvidenceDocuments,
} from "@/services/auditProgram/Execution";
import EvidenceTableDocument from "@/views/auditProgram/execution/evidence/EvidenceTableDocument";
import Document from "@/models/Document";
import ConfirmationDialog from "@/common/template/ConfirmModalDesign";

const EvidenceDocumentCrud = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const dataDocument = await getEvidenceDocuments(slugAuditTest);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteEvidenceDocuments(docToDelete.id);
    ...
    },
    {
      ...
      renderCell: (params) => (
        <IconButton
          onClick={async () => {
            try {
              const response = await getEvidenceDownloadDocuments(
                params.row.id
              );
  ...
  return (
    <>
      <EvidenceTableDocument
        title={`Lista de Evidencia - ${auditTest}`}
        rows={rows}
        columns={columns}
        limit={limitPage}
        rowHeight={"auto"}
...
export default EvidenceDocumentCrud;
