...
import {
  updateFinding,
  getFindingBySlug,
} from "@/services/auditProgram/Execution";
import {
...
const FindingUpdate = () => {
  ...
  const { slugAuditProgram, slugAuditTest, slugFinding } = useParams();
  ...
  useEffect(() => {
    fetchFinding();
  }, [slugFinding, setValue]);

  const fetchControls = async (findingData) => {
    try {
      const data = await getControlsAuditTestBySlug(slugAuditTest);
      const allControls = data.map((item) => new Control(item));
      const selected = allControls.filter((control) =>
        findingData.controls.some(
          (FindingsControls) => FindingsControls.id === control.id
        )
      );
  ...
  const fetchFinding = async () => {
    try {
      const data = await getFindingBySlug(slugFinding);
      const findingData = new Finding(data);
      fetchControls(findingData);
      setValue("title", findingData.title);
      setValue("observations", findingData.observations);
      setValue("root_cause", findingData.root_cause);
      setValue("possible_consequences", findingData.possible_consequences);
      setValue("recommendations", findingData.recommendations);
      setValue("finding_type", findingData.finding_type);
      setValue("classification", findingData.classification);
      setValue("status", findingData.status);
    } catch (error) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      setOpenConfirmModalDesign(false);
      const successMessage = await updateFinding(slugFinding, formData);
      setSuccess(successMessage);
      setSnackbarOpen(true);
      setTimeout(
        () =>
          navigate(
            `/audit-programs/manage-audit-programs/execution/${slugAuditProgram}/manage-findings/${slugAuditTest}/crud`
          ),
        2000
      );
    } catch (error) {
      setError(error.message || "Error al actualizar el hallazgo.");
    }
  };
  ...
  return (
    <Box display="flex" justifyContent="center" alignItems="center">
      <Paper elevation={10} sx={{ padding: 4, width: 500 }}>
        <Stack spacing={3} alignItems="center">
          ...
          <Typography variant="body2" color="text.secondary" textAlign="center">
            Llena los datos para actualizar el Hallazgo.
          </Typography>
          ...
          >
            ...
            <ButtonDesign>Actualizar Hallazgo</ButtonDesign>
          </form>
      ...
      <ConfirmModalDesign
        ...
        title="Confirmación de actualizar Hallazgo"
        message="¿Estás seguro de que deseas actualizar el hallazgo con esos datos?"
