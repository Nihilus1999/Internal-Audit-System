...
import { updateExecutionStatus } from "@/services/auditProgram/Execution";
import { statusPhasesOptions } from "@/utils/HelpersLib";

export default function ExecutionStatusModal({ open, onClose, onStatusSuccess}) {
  const {
    handleSubmit,
    control,
    setValue,
  ...
  const fetchAuditProgram = async () => {
    try {
      setLoading(true);
      const auditProgram = await getAuditProgramBySlug(slugAuditProgram);
      setValue("status", auditProgram.execution_status);
    } catch (error) {
  ...
  const onSubmit = async (formData) => {
    try {
      const message = await updateExecutionStatus(formData, slugAuditProgram);
