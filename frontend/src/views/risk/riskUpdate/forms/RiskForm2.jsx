...
const RiskForm2 = () => {
  ...
  const fetchProcess = async (riskData) => {
    try {
      const data = await getActiveProcess();
      const risk = data.map((item) => new Process(item));
      const allRisk = risk.map((u) => new Process(u));
      const selected = allRisk.filter((risk) =>
        riskData.processes.some((riskData) => riskData.id === risk.id)
      );
      setProcesses(allRisk);
      setValue(
        "ids_process",
  ...
  return (
    ...
    >
      <Paper elevation={5}>
        <Stack spacing={3} alignItems="center" sx={{ padding: 4, width: 600 }}>
          <Typography variant="h5" fontWeight="bold" textAlign="center">
            Procesos afectados por este riesgo
          </Typography>
          <Typography variant="body2" color="text.secondary" textAlign="center">
            Agrega los procesos que estén afectados por este riesgo.
          </Typography>
      ...
      <ConfirmModalDesign
        ...
        title="Confirmación de actualizar de procesos"
        message="¿Estás seguro de que deseas actualizar los procesos de este riesgo?"
